...
# EDA를 위한 그래프
with(CancerData, ggpairs(CancerData[, meanRange], aes(col = diagnosis)))
with(CancerData, ggpairs(CancerData[, seRange], aes(col = diagnosis)))
with(CancerData, ggpairs(CancerData[, worstRange], aes(col = diagnosis)))

# 통계량 EDA
psych::describe(CancerM[,totalRange])
# 근데 생각해보니까, 자료 자체가 mean, se, worst여서
# 데이터 자체가 이미 표본통계량임.
# 각 관측값의 value를 해석하자면, id 842302를 예를 들자면
# 1. 이 사람의 몸에 있는 관측한 모든 암세포들에 대해서
...
# MVN을 기반으로 Classification을 진행할 때 LDA보다 QDA가 필요함을 알 수 있음.
...
# MVN based Model에 대해 평가(Cross Validation)
# Fold 생성
k <- 10  # Fold 수
...
# -> 밑의 Case1,,...Case4를 비교한 결과로 추론한 결과.
...
# 그냥 데이터셋 group의 feature가 너무 명확해서 나온 정확도라고 생각됨.
...
# ML
# without / with outliers, normal
# 1. SVM
svm <- c(1.0, 0.5888157894736843)
# 2. Logistic
logis <- c(1.0, 0.625814536340852)
# 3. K-Nearest
kn <- c(1.0, 0.6011904761904762)
# 4. Random Forest
rf <- c(1.0, 0.6031015037593985)
# Gaussian Naive Bayes
gnb <- c(1.0, 0.638157894736842)
Classify_method <- c("Normal Classification","SVM", "Logistic Regression", 
  "K-Nearest Neighbors", "Random Forest", "Gaussian Naive Bayes")
acc <- c(mean(kfold_list4),svm[2],logis[2],kn[2],rf[2],gnb[2],
         mean(kfold_list3),svm[1],logis[1],kn[1],rf[1],gnb[1])
dev.off()
data.frame("Classify_method" = rep(Classify_method, 2),
           "isNormalized" = factor(rep(c(0,1), each=6)),
           "Accuracy" = round(acc, 3)) %>% gridExtra::grid.table()
## 정규화를 거칠 때 outlier detection은 명백히 유효했던 것으로 보임.
################################################
